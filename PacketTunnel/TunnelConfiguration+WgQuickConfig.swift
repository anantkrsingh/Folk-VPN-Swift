...
// WireGuardKit SPM product (only WireGuardKit/WireGuardKitC/WireGuardKitGo are), so the
// PacketTunnel extension needs its own copy to turn the backend's wg-quick config text
// into a TunnelConfiguration for WireGuardAdapter.
